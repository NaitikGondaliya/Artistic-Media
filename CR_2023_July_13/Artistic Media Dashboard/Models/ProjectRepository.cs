...
namespace Artistic_Media_Dashboard.Models
{
    public class ProjectRepository
    {
        ...
        public bool AddProjectAttachment(long projectId, int attachmentType, string filepath,string filename)
        {
        ...
        public List<DashboardResponse> DashboardData()
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                lstProject.Add(
                    ...
                    new ProjectModel
                    {
                        ...
                        ProjectDate = Convert.ToDateTime(dr["ProjectDate"]),
                        ProjectNumber = Convert.ToInt32(dr["ProjectNumber"]),
                        DispoDisc = Convert.ToString(dr["DispoDisc"]),
                        DispoFileName = Convert.ToString(dr["DispoFileName"]),
                        DispoAttachment = dr.IsNull("DispoAttachment") ?
                        string.Empty : HttpContext.Current.Request.Url.ToString().Contains("Sedinte")
            ...
            foreach (ProjectModel objProjectModel in lstProject)
            {
                ...
                List<tinyAttachmentModel> tempAttachmentName = new List<tinyAttachmentModel>();
                //Bind EmpModel generic list using dataRow     
                foreach (DataRow dr in dt1.Rows)
                {
                    // tempAttachmentName.Add(HttpContext.Current.Request.Url.ToString() + Convert.ToString(dr["AttachmentName"]));
                    //tempAttachmentName.Add(HttpContext.Current.Request.Url.ToString().Contains("Sedinte")
                    ...
                    // tempAttachmentName.Add(HttpContext.Current.Request.Url.ToString() + Convert.ToString(dr["AttachmentName"]));
                    tempAttachmentName.Add(
                        new tinyAttachmentModel()
                        {
                            strurl = HttpContext.Current.Request.Url.ToString().Contains("Sedinte")
                        ? (HttpContext.Current.Request.Url.ToString().Replace(HttpContext.Current.Request.Url.LocalPath, string.Empty) + "/" + Convert.ToString(dr["AttachmentName"])) :
                        (HttpContext.Current.Request.Url.ToString() + Convert.ToString(dr["AttachmentName"])),
                            filename = Convert.ToString(dr["filename"])
                        });
                    ...
                    {
                        objProjectModel.lstAttachments = new List<tinyAttachmentModel>();
                    }
        ...
        public long AddReleaseDisposition(ReleaseDispositionModel obj)
        {
            ...
            com.Parameters.AddWithValue("@DispoDesc", obj.DispoDisc);
