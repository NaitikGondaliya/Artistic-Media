...
namespace Artistic_Media_Dashboard.Controllers
{
    public class HomeController : Controller
    {
                    ...
                    if (Request.Files.Count > 0 && !string.IsNullOrWhiteSpace(filename))
                    {
                        foreach (string objfile in Request.Files)
                        {
                            ...
                            if (_file.ContentLength > 0 && supportedTypes.Contains(fileExt))
                            {
                                ...
                                ProjectRepository repository = new ProjectRepository();
                                if (repository.AddProjectAttachment(projectid, attachmentType, $"UploadedFiles/{DateTime.Now.ToString("MMddyyyy")}/{projectid}/{filename}.{fileExt}"))
                                {
                                    _file.SaveAs(_path);
                                }
        ...
        public string UploadReleaseDisposition(DateTime dispodate, string filename, HttpPostedFileBase file)
        {
            if (Session["UserID"] == null)
            {
                RedirectToAction("Login", "Home");
                    ...
                    if (Request.Files.Count > 0 && !string.IsNullOrWhiteSpace(filename))
                    {
                        foreach (string objfile in Request.Files)
                        {
                            ...
                            if (_file.ContentLength > 0 && supportedTypes.Contains(fileExt))
                            {
                                string _FileName = Path.GetFileName(_file.FileName);
                                _FileName = _FileName.Replace(" ", "_");
                                _path = Path.Combine(Server.MapPath("~/UploadedFiles/ReleaseDisposition"), $"{filename}.{fileExt}");
                                Session["Path"] = _path;
                                ...
                                ProjectRepository repository = new ProjectRepository();
                                if (repository.AddReleaseDisposition(new ReleaseDispositionModel() {AttachmentName= $"UploadedFiles/ReleaseDisposition/{filename}.{fileExt}",DispoDate=dispodate,ReleaseDispositionId=0 })>0)
                                {
                                    _file.SaveAs(_path);
                                }
                                message = "File Uploaded Successfully!!";
